...
describe('Sketchy', () => {
  ...
  describe('randomize', () => {
    ...
    it('doit retourner un tableau de point sensiblement identique', () => sketchy.randomize(POINTS).forEach((point, index) => expect(point).toEqual(jasmine.any(Array)) && expect(Math.abs(point[0] - POINTS[index][0]) <= 0.5).toBeTruthy() && expect(Math.abs(point[1] - POINTS[index][1]) <= 0.5).toBeTruthy()))
    it('doit pouvoir paramétrer la force du bruit', () => expect(sketchy.randomize(POINTS, {
      noise: 0.5
    })).not.toEqual(POINTS))
    it('doit retourner un tableau identique si le bruit est nul', () => expect(sketchy.randomize(POINTS, {
      noise: 0
    })).toEqual(POINTS))
  ...
  describe('getPointsFromSvgPath', () => {
    ...
    it('doit retourner un tableau', () => expect(sketchy.getPointsFromSvgPath(rsc.SQUARE.svg.g.g.path['@_d'])).toEqual(jasmine.any(Array)))
  ...
  describe('getPointsFromSvgPoints', () => {
    let data
    beforeEach(() => {
      data = sketchy.getPointsFromSvg(rsc.ONE_POLYLINE)
    })
  ...
  describe('getPathsFromSvg', () => {
    ...
    describe('SQUARE', () => {
      it('doit pouvoir retourner tous les chemins de SQUARE', () => expect(sketchy.getPathsFromSvg(rsc.ONE_PATH)).toEqual(jasmine.any(Array)))
      it('doit retourner 1 chemin', () => expect(sketchy.getPathsFromSvg(rsc.ONE_PATH)).toEqual(['M 0,0 l 1,1']))
    }
    )
    describe('MAP', () => {
      it('doit pouvoir retourner tous les chemins de MAP', () => expect(sketchy.getPathsFromSvg(rsc.ONE_PATH)).toEqual(jasmine.any(Array)))
      it('doit retourner 1 chemin', () => expect(sketchy.getPathsFromSvg(rsc.ONE_PATH)).toEqual(['M 0,0 l 1,1']))
    }
    )
  }
